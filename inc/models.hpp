...
void Model::loadModel(std::string path)
{
    ...
    const aiScene* scene = importer.ReadFile(
        path, aiProcess_Triangulate); // 如果已经执行了stbi_set_flip_vertically_on_load(true);，则不需要再次翻转y轴
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    // 3. normal maps
    std::vector<Texture> normal_maps = loadMaterialTextures(material, aiTextureType_HEIGHT, "normal");
    textures.insert(textures.end(), normal_maps.begin(), normal_maps.end());
    // 4. height maps
    std::vector<Texture> height_maps = loadMaterialTextures(material, aiTextureType_AMBIENT, "height");
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        mat->GetTexture(
            type,
            i,
            &str); // 注意，我们假设了模型文件中纹理文件的路径是相对于模型文件的本地(Local)路径，比如说与模型文件处于同一目录下。我们可以将纹理位置字符串拼接到之前（在loadModel中）获取的目录字符串上，来获取完整的纹理路径（这也是为什么GetTexture函数也需要一个目录字符串）
        bool skip = false;
        for (auto& texture : textures_loaded)
            if (std::strcmp(str.C_Str(), texture.path.data()) == 0)
            {
